...
MODE = 'training'
CKPT = 'ckpt/weights_ori_H36_100.ckpt'
...
class ori_esti:
	...
	TEST_SIZE = 5000
	MODEL_DIR = 'model/'
	IMG_PATH = 'H36/training/'
	IMG_PATH_TEST = 'H36/training/'
	IMG_PATH_REAL = 'crop_resize/'
	LABEL_FILE = 'H36/labels/labels_training_H36_8class.txt'
	LABEL_FILE_TEST = 'H36/labels/labels_test_H36_8class.txt'
	LABEL_FILE_REAL = 'labels/labels_training_depth.txt'
	CKPT_NAME_PREFIX = 'ckpt/weights_ori_H36_'
	BATCH_SIZE=100
	...
	MAX_DISTANCE = 4000
	LOG_TRAIN = 'log_train_H36.txt'
	LOG_TEST = 'log_test_H36.txt'
	LOG_CONFU = 'log_confu_H36.txt'
	...
	def train(self):
		...
		for step in range(self.TRAIN_SIZE*self.NUM_EPOCHS // self.BATCH_SIZE):
			for i in range(self.BATCH_SIZE):			
				...
				img[i,:,:,:] = self.pre_proc(cv2.imread(self.IMG_PATH + str(self.shuffled_arange[idx]) +'.jpeg'))
			...
			if step % self.TEST_FREQUENCY == 0:
				...
				for ii in range(self.TEST_SIZE//self.BATCH_SIZE):
					...
					for jj in range(self.BATCH_SIZE):
						...
						img_test[jj,:,:,:] = cv2.resize(cv2.imread(self.IMG_PATH_TEST + str(self.shuffled_arange[idx]) +'.jpeg'),(self.WH,self.WH))/255.0
